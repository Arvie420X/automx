...
def home_view(request):
    ...
    user_liked_listings = LikedListing.objects.filter(
        profile=request.user.profiles).values_list('listing')
    ...
    if request.method == 'POST':
        try:
            ...
            if listing_form.is_valid() and location_form.is_valid():
                ...
                listing.seller = request.user.profiles
            ...
            print(e)
            messages.error(request, f'{e} - An error occured while posting the listing.')
...
def like_listing_view(request, id):
    ...
    liked_listing, created = LikedListing.objects.get_or_create(
        profile=request.user.profiles, listing=listing)
